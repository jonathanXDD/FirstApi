...
ROUTING
'''
@app.post("/todos",response_modle=TodoResponse )
def create_todo(todo:TodoCreate,db:Session=Depends(get_db())):
    db_todo=Todo(**todo.dect())
    db.add(db_todo)
    db.commit()
    db.refresh(db_todo)
    return db_todo
@app.get("/todos",response_modle=list[TodoResponse])
def read_todos(db: Session =Depends(get_db())):
    return db.query(Todo).all()


@app.get("/todo/{todo_id}",response_modle=TodoResponse)
...
def update_todo(todo_id:int,todo:TodoCreate,db:Session=Depends(get_db())):
    ...
    if not db_todo:
        raise HTTPException(status_code=404,details="Todo not found")
    for key,value in todo.dect().items():
